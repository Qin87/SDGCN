...
def hermitian_decomp_sparse(pos_edges, neg_edges, size, q=0.25, norm=True, laplacian=True,
                            max_eigen=2, gcn_appr=False, edge_weight=None, phase_norm=False):
    ...
    if laplacian:
        ...
        if phase_norm:
            data = phase.toarray().reshape(1,-1)
            data = [e/(abs(e)+0.00001) for e in data[0]]
            pha, row, col =[], [], []
            for i,j in enumerate(data):
                if 0.9 <= abs(j):
                    ...
                    col.append(i % size)
                    pha.append(j)
            Theta = coo_matrix((pha, (row, col)), shape=(size, size), dtype=np.complex64)

        if norm:
            D = diag
        else:
            d = np.sum(A_sym, axis=0)
            D = coo_matrix((d, (np.arange(size), np.arange(size))), shape=(size, size), dtype=np.float32)
