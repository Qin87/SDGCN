...
def sub_adj(edge_index, prob, seed):
    ...
    #return sub_train.T, sub_test.T, sub_test.T
...
def label_pairs_gen_dir(pos, neg):
    pairs = torch.cat((pos, neg), axis=-1)
    rng = np.random.default_rng(1)
    perm = rng.permutation(len(pairs[0]))
    pairs = pairs[:, perm]

    forward = pairs[:, :int(len(pairs[1])/2)]
    reverse = pairs[:, int(len(pairs[1]) / 2):]
    reverse = torch.cat((reverse[1,:].view(1,-1), reverse[0,:].view(1,-1)),0)
    pairs = torch.cat((forward, reverse),1)
    label = np.r_[np.ones(len(forward[0])), np.zeros(len(reverse[0]))]
    return pairs, label

def label_pairs_gen_link(pos, neg):
    max_idx = max(pos.max(), neg.max()).item()
    pairs = torch.cat((pos, neg), axis=-1)
    rng = np.random.default_rng(1)
    perm = rng.permutation(len(pairs[0]))
    pairs = pairs[:, perm]

    pos_link = pairs[:, :int(len(pairs[1])/2)]
    neg_link = pairs[:, int(len(pairs[1]) / 2):]

    neg_link_dst = torch.tensor(np.random.choice(list(range(1, max_idx)), len(neg_link[0, :]))).to(neg.device)
    neg_link = torch.cat((neg_link[0,:].view(1,-1), neg_link_dst.view(1,-1)),0)

    pairs = torch.cat((pos_link, neg_link),1)
    label = np.r_[np.ones(len(pos_link[0])), np.zeros(len(neg_link[0]))]
    return pairs, label
...
def generate_dataset_2class(pos_edge_index, neg_edge_index, splits=5, test_prob=0.2, instance_balancing=True, ratio=3):
    ...
    for i in range(splits):
        train_pos, val_pos, test_pos = sub_adj(pos_edge_index, prob=test_prob, seed=i * 10)
        train_neg, val_neg, test_neg = sub_adj(neg_edge_index, prob=test_prob, seed=i * 10)
        datasets[i] = {}
...
def generate_dataset_2class_dir(pos_edge_index, neg_edge_index, splits=5, test_prob=0.2, instance_balancing=True, ratio=3):
    datasets = {}
    ...
    for i in range(splits):
        ...
        pairs, label = label_pairs_gen_dir(train_pos, train_neg)
        perm = rng.permutation(len(pairs[0]))
        datasets[i]['train']['pairs'] = pairs[:, perm].detach().to('cpu').numpy().T
        datasets[i]['train']['label'] = label[perm]
        ...
        # validation data
        datasets[i]['validate'] = {}
        pairs, label = label_pairs_gen_dir(val_pos, val_neg)
        ...
        # test data
        datasets[i]['test'] = {}
        pairs, label = label_pairs_gen_dir(test_pos, test_neg)
        perm = rng.permutation(len(pairs[0]))
        datasets[i]['test']['pairs'] = pairs[:, perm].detach().to('cpu').numpy().T
        datasets[i]['test']['label'] = label[perm]
...
def generate_dataset_2class_link(pos_edge_index, neg_edge_index, splits=5, test_prob=0.2, instance_balancing=True, ratio=3):
    datasets = {}
    ...
    for i in range(splits):
        ...
        pairs, label = label_pairs_gen_link(train_pos, train_neg)
        perm = rng.permutation(len(pairs[0]))
        datasets[i]['train']['pairs'] = pairs[:, perm].detach().to('cpu').numpy().T
        datasets[i]['train']['label'] = label[perm]
        ...
        # validation data
        datasets[i]['validate'] = {}
        pairs, label = label_pairs_gen_link(val_pos, val_neg)
        ...
        # test data
        datasets[i]['test'] = {}
        pairs, label = label_pairs_gen_link(test_pos, test_neg)
        perm = rng.permutation(len(pairs[0]))
        datasets[i]['test']['pairs'] = pairs[:, perm].detach().to('cpu').numpy().T
        datasets[i]['test']['label'] = label[perm]
...
if __name__ == "__main__":
    data = load_directed_signed_graph_link(root='./dataset/data/' + 'Epinions')
